...
export const EditBookingForm = ({ booking, onSuccess, onCancel }) => {
  ...
  useEffect(() => {
    ...
    if (booking) {
      ...
      setIsKitchen(booking.isKitchen || false);
      setIsExtraBed(booking.isExtraBed || false);

      // Fetch related data
      if (booking.hotelID) {
        fetchCategories(booking.hotelID);
        if (booking.roomCategoryName) {
          fetchRooms(booking.hotelID, booking.roomCategoryName);
        }
      }
    }
  }, [booking]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload = {
        ...values,
        bookingID: booking.bookingID, // Keep original booking ID
        transactionId: values.transactionId || booking.transactionId,
        bookedByID: booking.bookedByID, // Keep original bookedByID
        nights,
        totalBill,
        statusID: booking.statusID, // Keep original status
        bookedBy: booking.bookedBy, // Keep original bookedBy
        isKitchen,
        isExtraBed,
        duePayment: totalBill - (values.advancePayment || 0),
        // Include additional required fields
        roomCategoryName: values.roomCategoryId,
      ...
      message.error(error.response?.data?.error || "Failed to update booking");
      console.error("Booking update error:", error.response?.data);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Form form={form} layout="vertical" onValuesChange={calculateTotalAmount}>
